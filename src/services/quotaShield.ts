...
import { Match, QuotaMetrics, SportType } from '../types.js';
...
export class QuotaShieldService {
  ...
  public async getLiveMatches(sport: string = 'soccer'): Promise<Match[]> {
    this.totalRequests++;
    ...
    try {
      ...
      console.warn(`[QUOTA SHIELD] Live ingest failed: ${(err as Error).message}. Returning resilient fallback cache.`);
    }
  ...
  private async fetchFromOddsApi(sportKey: string): Promise<Match[]> {
    ...
    if (remaining) this.remainingCalls = parseInt(remaining, 10);
    if (used) this.usedCalls = parseInt(used, 10);
    ...
    return json.slice(0, 15).map((item: any, idx: number) => {
      ...
      return {
        ...
        leagueId: (item.sport_key?.includes('epl') ? 'EPL' : item.sport_key?.includes('la_liga') ? 'LALIGA' : item.sport_key?.includes('champs') ? 'UCL' : 'EPL') as any,
        ...
        isLive,
        handicapTeam: (homeHdp.startsWith('-') ? 'home' : 'away') as any,
  ...
  private getFallbackMatches(sport: string): Match[] {
    const now = new Date();
    return [
      // 1. Live In-Play Match
      {
        ...
        matchId: 'sb_fb_01',
        leagueId: 'LALIGA' as any,
        leagueName: 'Giải Tây Ban Nha (La Liga)',
        scoreHome: 1,
        scoreAway: 2,
        liveTime: "78' (H2)",
        isLive: true,
        handicapTeam: 'home' as any,
        homeHandicap: '0.00',
        homeOdds: 0.98,
        awayHandicap: '0.00',
        awayOdds: -0.92,
