...
import { Bet, BetSelection, BetStatus, BetType } from '../types.js';
import { walletLedger } from './walletLedger.js';
import { quotaShield } from './quotaShield.js';

export class BetQueueService {
  private queue: Map<string, Bet> = new Map();
  private userSubMarketStakes: Map<string, number> = new Map(); // Tracks anti-hedging & 300 pt container cap
  private platformTier: 1 | 2 | 3 = 1;

  public getPlatformTier(): 1 | 2 | 3 {
    return this.platformTier;
  }

  public setPlatformTier(tier: 1 | 2 | 3): void {
  ...
  public enqueueBet(
    ...
    stake: number
  ): { success: boolean; bet?: Bet; error?: string } {
    ...
    let delaySeconds: number = CONFIG.DELAY_TIERS.TIER_1_STANDARD_SECONDS;
  ...
  private async processBet(betId: string): Promise<void> {
    ...
    for (const sel of bet.selections) {
      ...
      if (sel.type === 'Home') currentOdds = liveMatch.odds.homeWin;
      else if (sel.type === 'Away') currentOdds = liveMatch.odds.awayWin;
      else if (sel.type === 'Draw') currentOdds = liveMatch.odds.draw || sel.oddsAtPlacement;
