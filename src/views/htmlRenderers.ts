...
export function renderTaiXiuHtml(data: { line: string; state: any; isBettingOpen: boolean }): string {
  ...
  const historyBeads = (history || []).map((h: any) => {
...
export function renderDashboardHtml(data: {
  ...
  const dicePips: Record<number, string> = { 1: '⚀', 2: '⚁', 3: '⚂', 4: '⚃', 5: '⚄', 6: '⚅' };
    ...
    <div class="grid-3">
      ...
      <div class="card">
        ...
        <div id="dash-history-container" style="display: flex; gap: 6px; flex-wrap: wrap;">
          ${(taixiu.state.history || []).slice(0, 14).map((h: any) => `
