...
export class CockfightService {
    ...
    voidMatch(arenaId) {
        const arena = this.arenas.get(arenaId);
        if (!arena)
            return { success: false, message: 'Arena not found', arena: null };
        arena.phase = 'CLOSED';
        arena.status = 'CLOSED';
     ...
     */
    overrideArenaPhase(arenaId, phase, winner) {
        const arena = this.arenas.get(arenaId);
        if (!arena)
            return { success: false, arena: null };
        arena.phase = phase;
        arena.status = phase;
        ...
        else if (phase === 'SETTLING' || phase === 'RESULT_ANNOUNCED') {
