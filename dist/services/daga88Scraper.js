...
class Daga88ScraperService {
    ...
    async scrapeArena(arenaKey) {
        const config = ARENA_SLUGS[arenaKey];
        if (!config)
            return null;
        if (this.isFetching.has(arenaKey))
            return this.cache.get(arenaKey) || null;
        ...
        try {
            ...
            for (let daysAgo = 0; daysAgo <= 2; daysAgo++) {
                ...
                if (uuids.length === 0)
                    continue;
                const matches = uuids.map((uuid, i) => ({
                    matchNumber: i + 1,
     ...
     */
    async getVideosForArena(frontendArenaId) {
